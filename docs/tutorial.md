...
This tutorial introduces the capabilities of `factory-duck`. We'll start with a simple
factory for a hypothetical `User` model and gradually add to it. This tutorial may not
cover all aspects of `factory-duck`, but should serve as a good starting point.

### The `User` Factory

Let's start with a simple `User` factory, as we go on, we'll keep on modifying
this factory to add functionality and show how `factory-duck` works.

```javascript
import factory from 'factory-duck';
...
Whenever we need a `User` object now, we can just ask `factory-duck` to  build one for us.
That's awesome, but not very useful yet. All the objects we  get back from `factory-duck`
...
`factory-duck` has a solution: sequences. Instead of providing a hardcoded  value, we can
tell factory-duck to instead use a sequence. A slight modification to the model-factory
definition:
...
Now we get a new email address every time we ask `factory-duck` for a  `User` instance:
...
name 1`? `factory-duck` provides another goodie that we can use for a more 'realistic'
data: `chancejs`. You can learn more about `chancejs` [here](http://chancejs.com/).
`factory-duck` exposes a simple '#chance' api that can be easily used to populate fields
...
several models associated with each other there are a few ways `factory-duck` 
...
`factory-duck` will now create a `ProfileImage` instance and place its `id` attribute in
...
Now, we can tell `factory-duck` to associate multiple addresses with our `User` instances:
...
So far so good! We can already see `factory-duck` making our life easier to   build model
...
`factory-duck` allows you to define model factories with an initializer function instead
...
can be specified when requesting `factory-duck` to create an instance. Using
...
Now, when requesting `factory-duck` to create a `User` instance, we can do:
